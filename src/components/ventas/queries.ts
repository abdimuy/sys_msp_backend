...
export const QUERY_GET_VENTAS_BY_RUTA = `
  ${QUERY_GET_VENTAS_BY_CLIENTE_BASE}
  WHERE DOCTOS_CC.CONCEPTO_CC_ID = 5
  ORDER BY FECHA DESC
`;
...
export const QUERY_GET_ARTICULOS_BY_FOLIO = `
  SELECT
    DOCTOS_PV.DOCTO_PV_ID,
    DOCTOS_PV.FOLIO,
    DOCTOS_PV.FECHA,
    DOCTOS_PV.CLIENTE_ID,
    (DOCTOS_PV.IMPORTE_NETO + DOCTOS_PV.TOTAL_IMPUESTOS + DOCTOS_PV.IMPORTE_DONATIVO) AS IMPORTE
  FROM DOCTOS_PV
  WHERE DOCTOS_PV.FOLIO = ?
  ;
`;

export const QUERY_GET_ARTICULOS_ITEMS_BY_VENTA_ID = `
  SELECT
  ...
  WHERE DOCTOS_PV_DET.DOCTO_PV_ID = ? AND DOCTOS_PV_DET.POSICION > 0
  ORDER BY DOCTOS_PV_DET.POSICION
  ;
`;

export const QUERY_GET_VENTES_BY_CLIENTES_SIMPLIFICADO = `
  SELECT
    M.DOCTO_CC_ACR_ID,
  ...
  (
    ...
    WHERE DOCTOS_CC.CLIENTE_ID IN (100362)
