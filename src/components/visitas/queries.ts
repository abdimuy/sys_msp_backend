...
export const GET_PAGOS_AND_VISITAS_BY_FECHA = (dateInit: string, dateEnd: string, zonaClienteId: number) => `
    ...
    FROM
        MSP_PAGOS_RECIBIDOS
        INNER JOIN DOCTOS_CC ON DOCTOS_CC.DOCTO_CC_ID = MSP_PAGOS_RECIBIDOS.DOCTO_CC_ID
    ...
    WHERE
        MSP_PAGOS_RECIBIDOS.FECHA >= TIMESTAMP '${dateInit}'
        AND MSP_PAGOS_RECIBIDOS.FECHA <= TIMESTAMP '${dateEnd}';
`