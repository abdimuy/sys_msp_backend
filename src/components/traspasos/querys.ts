...
export const QUERY_GET_TRASPASOS = `
  SELECT 
    ...
    D.CANCELADO,
    D.USUARIO_CREADOR,
    REPLACE(LEFT(CAST(D.FECHA_HORA_CREACION AS VARCHAR(30)), 23), ' ', 'T') || '-06:00' AS FECHA_HORA_CREACION,
...
export const QUERY_VALIDAR_EXISTENCIAS = `
  SELECT 
    A.ARTICULO_ID,
    CART.CLAVE_ARTICULO AS CLAVE,
    A.NOMBRE,
    COALESCE(EXIS.EXISTENCIAS, 0) AS EXISTENCIA,
    COALESCE(EXIS.EXISTENCIAS, 0) AS EXISTENCIA_DISPONIBLE
  FROM ARTICULOS A
  LEFT JOIN CLAVES_ARTICULOS CART ON CART.ARTICULO_ID = A.ARTICULO_ID AND CART.ROL_CLAVE_ART_ID = 17
  LEFT JOIN (
    SELECT
      ARTICULO_ID,
  ...
  ) EXIS ON EXIS.ARTICULO_ID = A.ARTICULO_ID
  WHERE A.ARTICULO_ID = ?
`;

export const QUERY_GET_CLAVE_ARTICULO = `
  SELECT 
    CART.CLAVE_ARTICULO AS CLAVE,
    ART.NOMBRE,
    ART.ARTICULO_ID
