...
export const QUERY_GET_ARTICULOS_BY_FOLIO = (folios: string) => `
  ...
  WHERE DOCTOS_PV.TIPO_DOCTO = 'V'
    AND DOCTOS_PV.FOLIO IN ('${folios}')
    AND DOCTOS_PV.APLICADO = 'S'
    ...
    (
        ...
        FROM
            ...
            INNER JOIN (
			    ...
			    FROM
			        SALDOS_CC
			    INNER JOIN CLIENTES ON CLIENTES.CLIENTE_ID = SALDOS_CC.CLIENTE_ID
			    WHERE 
			        ZONA_CLIENTE_ID = ?
			    GROUP BY 
			        CLIENTES.CLIENTE_ID,
                    ZONA_CLIENTE_ID
			    HAVING 
			        (SUM(CARGOS_CXC) - SUM(CREDITOS_CXC) - SUM(CREDITOS_CXC_XLIB)) > 0
			) CLIENTES_OK ON DOCTOS_CC.CLIENTE_ID = CLIENTES_OK.CLIENTE_ID
        WHERE
            DOCTOS_CC.CANCELADO = 'N'
            AND CLIENTES_OK.ZONA_CLIENTE_ID = 21563
        GROUP BY
            IMPORTES_DOCTOS_CC.DOCTO_CC_ACR_ID
    ) M
    INNER JOIN DOCTOS_CC ON DOCTOS_CC.DOCTO_CC_ID = M.DOCTO_CC_ACR_ID
...
export const QUERY_GET_VENTA_BY_ID = `
SELECT
M.DOCTO_CC_ACR_ID,
DOCTOS_CC.DOCTO_CC_ID,
DOCTOS_CC.FOLIO,
...
//         WHERE
//             DOCTOS_CC.CANCELADO = 'N'
//             AND CLIENTES_OK.ZONA_CLIENTE_ID = ?
//         GROUP BY
//             IMPORTES_DOCTOS_CC.DOCTO_CC_ACR_ID
//     ) M
//     INNER JOIN DOCTOS_CC ON DOCTOS_CC.DOCTO_CC_ID = M.DOCTO_CC_ACR_ID
